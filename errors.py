...
def unknown_command(user: server.UserConnection, command: str) -> None:
...
def not_registered(user: server.UserConnection) -> None:
    """
...
def no_motd(user: server.UserConnection) -> None:
    """Sent when server cannot find the Message of the Day."""
    message = f"422 {user.nick} :MOTD File is missing"
    user.send_que.put((message, "mantatail"))


def erroneus_nickname(user: server.UserConnection, new_nick: str) -> None:
    message = f"432 {new_nick} :Erroneous Nickname"
    user.send_que.put((message, "mantatail"))


def nick_in_use(user: server.UserConnection, nick: str) -> None:
    """Sent when a Nick that a user tries to establish is already in use."""
    message = f"433 {user.nick} {nick} :Nickname is already in use"
    user.send_que.put((message, "mantatail"))


def no_nickname_given(user: server.UserConnection) -> None:
    message = "431 :No nickname given"
    user.send_que.put((message, "mantatail"))


def no_such_nick_channel(user: server.UserConnection, channel_or_nick: str) -> None:
...
def not_on_channel(user: server.UserConnection, channel_name: str) -> None:
...
def user_not_in_channel(
    user: server.UserConnection, target_usr: server.UserConnection, channel: server.Channel
) -> None:
    """
...
def cannot_send_to_channel(user: server.UserConnection, channel_name: str) -> None:
    """
...
def banned_from_chan(user: server.UserConnection, channel: server.Channel) -> None:
...
def no_operator_privileges(user: server.UserConnection, channel: server.Channel) -> None:
    """
...
def no_recipient(user: server.UserConnection, command: str) -> None:
...
def no_text_to_send(user: server.UserConnection) -> None:
    """
...
def unknown_mode(user: server.UserConnection, unknown_command: str) -> None:
...
def no_origin(user: server.UserConnection) -> None:
    """

            ...
            while not request.endswith(b"\n"):
                ...
                if request_chunk:
                    ...
                    if user is not None:
                        ...
                        command.delete_user_from_server_users(state, user)
                    else:
                        print("Disconnected.")
                    return
            ...
            for line in split_on_new_line(decoded_message)[:-1]:
                ...
                if user is None:
                    if verb_lower == "user":
                        ...
                        if message.lower() in state.users:
                            user_socket.sendall(command.error_nickname_in_use(message))
                        else:
                            _nick = message
                            state.users.add(_nick.lower())
                    else:
